...
document.querySelector('#presentation-form').addEventListener('submit',async e=>{
  ...
  if(response.ok){e.currentTarget.reset();show('Apresentação adicionada.');load()}else show(response.status===401?'PIN incorreto.':'Não foi possível adicionar.');
});
document.querySelector('#presentation-list').addEventListener('click',async e=>{
  ...
  if(activateId){
    ...
    show(response.ok?'Apresentação liberada para votação!':body.message||'PIN incorreto.');return load();
  }
  const removeId=e.target.dataset.remove;if(!removeId)return;
  const response=await fetch(`/api/admin/presentations/${removeId}`,{method:'DELETE',headers:headers()});
  show(response.ok?'Apresentação excluída.':response.status===401?'PIN incorreto.':'Não é possível excluir agora.');load();
});
document.querySelector('#stop-session').addEventListener('click',async()=>{
  ...
  show(response.ok?'Apresentação retirada da votação.':'PIN incorreto.');load();
});
document.querySelector('#finish-event').addEventListener('click',async()=>{
  ...
  show(response.ok?'Evento finalizado e ranking liberado.':'PIN incorreto.');load();
});
document.querySelector('#clear-votes').addEventListener('click',async()=>{
  ...
  const response=await fetch('/api/admin/votes',{method:'DELETE',headers:headers()});show(response.ok?'Votos apagados.':'PIN incorreto.');load();
});
function show(message){feedback.textContent=message;feedback.classList.add('show');setTimeout(()=>feedback.classList.remove('show'),2600)}
function escapeHtml(value){const el=document.createElement('div');el.textContent=value;return el.innerHTML}
const publicUrl=new URL('/',location.href).href;
document.querySelector('#public-link').href=publicUrl;document.querySelector('#public-link').textContent=publicUrl;
